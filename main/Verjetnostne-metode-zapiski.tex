...
\begin{defn}[$(\varepsilon,\delta)$-approximation]
  A random algorithm gives a $(\varepsilon,\delta)$-approximation for value $v$ if the output $X$ satisfies:
  \begin{equation*}
...
\begin{theorem}
  ...
  If $m \geq \frac{3 \ln\left(\frac{2}{\delta}\right)}{\varepsilon^2 \mu}$, then $P_r(|Y-\mu| \geq \varepsilon \mu) \leq \delta$
...
\begin{pro} \text{} \\
  ...
  \begin{align*}
    P_r(|Y-\mu| \geq \varepsilon \mu) &= P_r\left(\left|\frac{X}{m}-\mu\right| \geq \varepsilon \mu\right) \\
...
\frac{3 \ln\left(\frac{2}{\delta}\right)}{\varepsilon^2} \cdot \frac{2^n}{x(F)}$ \\
...
\begin{defn} \text{} \\
  ...
  $P_r(G \in G(n,p) \text{ has property}) \stackrel{n \longrightarrow \infty}{\to} 1$.
\end{defn}
\begin{claim} \text{} \\
  Let $p$ be constant.
...
\begin{itemize}[label={}]
  \item Init: $k = \lceil \frac{2}{\varepsilon} \rceil, t = \lceil \log_2 \left(\frac{1}{\delta}\right) \rceil$.
...
\begin{align*}
  P(\overline{d} \geq 3d) &= P(\text{at least} \left\lceil \frac{k}{2} \right\rceil \overline{d}-s \text{ are } \geq 3d) \\
...
\begin{itemize}
  ...
  \item $P(out(V, x, r, P^{'}) = 1) \leq \delta$ for every $P^{'}, \delta_s \in [0, 1)$.
\end{itemize}
...
Zero-knowledge:
\begin{itemize}
  \item informal: verifier learns nothing behind the claim.
\end{itemize}
\begin{ex} \text{} \\
  Input: $G$ graph, \\
  $f(G) = \begin{cases}
